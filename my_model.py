            ...
            for batch in self.s_gen.gen(self.batch_size):
                ...
                for idx, output, ground in zip(range(len(grounds)), step_outputs, grounds):
                    ...
                    if distance_loaded:
                        ...
                        lev.append(num_incorrect)
                        dev = max(output_valid, ground_valid)
                        lev_reg.append(float(num_incorrect) / len(dev))
                        if self.visualize:
                ...
                average_lev = float(sum(lev))/len(lev)
                average_lev_reg = float(sum(lev_reg))/len(lev_reg)
                logging.info('%f out of %d correct, average distance %f, average regularized %f' %(num_correct, num_total, average_lev, average_lev_reg))
